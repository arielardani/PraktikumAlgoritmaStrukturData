...
public class BinaryTree04 {
    ...
    private Node04 addRekursifHelper(Node04 current, Mahasiswa04 mahasiswa) {
        if (current == null) {
    ...
    private void tampilIPKdiAtasHelper(Node04 current, double ipkBatas) {
        if (current != null) {
            tampilIPKdiAtasHelper(current.left, ipkBatas);
            if (current.mahasiswa.ipk > ipkBatas) {
                System.out.println("- " + current.mahasiswa.nama + " (IPK: " + current.mahasiswa.ipk + ")");
            }
